if(identical(Sys.getenv("NOT_CRAN"), "true") & .Machine$sizeof.pointer != 4){
  ...
  test_that("compareIRTmodels handles 1D 1PL-4PL bigIRT fits", {
    for(pl in 1:4){
      ...
      expect_true(all(c("source", "value", "referenceVal", "itemCurveRMSE") %in% colnames(cmp$itempars)))
      ...
      expect_true(all(is.finite(stats::na.omit(cmp$itempars$itemCurveRMSE))))
      ...
      expect_true(all(cmp$itempars[source == "truth", itemCurveRMSE] == 0))
