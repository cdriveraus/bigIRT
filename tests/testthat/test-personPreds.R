...
cores=2
  ...
  test_that("personPreds2PL", {
    ...
    predBetaStd <- predBeta * apply(dat$dat[,c('V1','V2')],2,sd) / sd(dat$dat$Ability)
    ...
    fit <- fitIRT(dat$dat,cores=cores,pl=2,plot=F,verbose=10,priors=T,
      ...
      # itemDat = itemdat,
      betaScale = 100,tol=1e-3,
      normalise = F,ebayes = T,ebayesmultiplier = 2)

    parsfit<-normaliseIRT(B = fit$itemPars$B,Ability = fit$personPars$X1,A = fit$itemPars$A)
    parstrue<-normaliseIRT(B = dat$B,Ability = dat$dat[!duplicated(id),Ability],A = dat$A)
