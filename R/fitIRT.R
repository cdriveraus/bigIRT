...
#'   print(fit$itemPars)
fitIRT <- function(dat,score='score', id='id', item='Item', scale='Scale',pl=1,personSlipping=FALSE,
  ...
  SlipMeandat=array(2,dim=c(length(unique(dat[[scale]])))),
  AbilitySD=array(10,dim=c(length(unique(dat[[scale]])))),
  SlipSD=array(1,dim=c(length(unique(dat[[scale]])))),
  ...
  sdat <- c(sdat,list(
    ...
    Slipdata=matrix(1,Nsubs,Nscales),
    ...
    fixedSlipMean=as.integer(!'slip' %in% estMeans & !'Slip' %in% estMeans & personSlipping),
    rowIndexPar=0L,
    ...
    personSlipping=as.integer(personSlipping)
  ))
