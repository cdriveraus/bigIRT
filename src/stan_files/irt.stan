...
vector[itemSpecificBetas ? (Nitems-NfixedA) : 1] invspAbeta[(Nitems-NfixedA) ? NitemPreds : 0];//regression weights for covariate effects on inverse softplus A params
vector[itemSpecificBetas ? (Nitems-NfixedB) : 1] Bbeta[(Nitems-NfixedB) ? NitemPreds : 0];//regression weights for covariate effects on B params
vector[itemSpecificBetas ? (Nitems-NfixedC) : 1] logitCbeta[(Nitems-NfixedC) ? NitemPreds : 0];//regression weights for covariate effects on logit C params
...
if(dopriors){
  for(i in 1:NitemPreds){
    if(size(invspAbeta)) invspAbeta[i,] ~ normal(0,1);
    if(size(Bbeta)) Bbeta[i,] ~ normal(0,1);
    if(size(logitCbeta)) logitCbeta[i,] ~ normal(0,1);
  }
  for(i in 1:Nscales){
    if(num_elements(Abilitybeta[i,])) Abilitybeta[i,] ~ normal(0,1);
