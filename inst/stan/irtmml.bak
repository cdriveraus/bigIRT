...
vector[Nscales] logAbilitySD; // standard deviation of the ability parameters.
...
vector[Nscales] AbilitySD = exp(logAbilitySD); //means of ability parameters
...
for(i in 1:Nobs){
  ...
  sAbility[i]= fixedAbilityLogical[id[i],scale[i]] ? Abilitydata[id[i],scale[i]] : AbilityMean[scale[i]];// + AbilityMean[Abilityparsscaleindex[Abilityparsindex[id[i],scale[i]]]];
  ...
  //p[i]= ((1-score[i])+ scoreCoef[i] * e6[i]) * (1-integrateWeight);

  //sAbilityShift = sqrt(.5*AbilitySD[scale[i]]^2);
  for(ii in 1:(NintegratePoints)){ //but skip 0!
      p[i] +=  integrateWeights[ii] * (
      (1-score[i]) + scoreCoef[i] * (
        sC[i] + e4[i] * inv_logit(sA[i] * (
          (sAbility[i] + AbilitySD[scale[i]] * integratePoints[ii]) -
          sB[i]))));
  }
